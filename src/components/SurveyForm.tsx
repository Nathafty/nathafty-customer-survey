    ...
    <form onSubmit={handleSubmit} className="card space-y-4 sm:space-y-6">
      ...
      <div className="space-y-4 sm:space-y-6">
        ...
        <div>
          <label htmlFor="trashFrequency" className="block text-sm font-medium text-gray-700 mb-1">
            Quelle fréquence sortez-vous vos poubelles ? *
          </label>
          <select
            id="trashFrequency"
            name="trashFrequency"
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-3">
            En moyenne, combien de sacs/barils de déchets produisez-vous par semaine ? *
          </label>
        ...
        <div>
          <label htmlFor="mainConcern" className="block text-sm font-medium text-gray-700 mb-1">
            Qu&apos;est-ce qui vous dérange le plus dans la gestion actuelle de vos déchets ? *
          </label>
          <textarea
            id="mainConcern"
            name="mainConcern"
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">
            Payez-vous actuellement un service lié à la collecte des déchets ? *
          </label>
      ...
      {submitError && (
        <Alert variant="danger">
          <span className="font-medium">Erreur de soumission —</span> {submitError}
        </Alert>
      )}

      {/* Bouton de soumission */}
      <div className="flex justify-center pt-4">
        <Button type="submit" loading={isSubmitting} size="lg" className="w-full sm:w-auto sm:px-12">
          <Send className="w-4 h-4" aria-hidden="true" />
          {isSubmitting ? 'Envoi en cours...' : "Soumettre l'enquête"}
        </Button>
      </div>
    </form>
  );
